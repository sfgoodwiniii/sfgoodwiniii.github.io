...
function fetchLocalStorage(key, keyDefaultValue=null) {
    if (localStorage.getItem(key) === null) {
        ...
        console.debug("Value for " + key + " found in the local storage: " + localStorage.getItem(key));
...
function fetchObjectHTML(parentID, childPseudoID) {
    ...
    const childHTML = Array
        .from(parentHTML.children)
        .find((child) => child.innerHTML.toLowerCase() === childPseudoID.toLowerCase());
